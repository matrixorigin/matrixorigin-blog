...
In TAE, a table contains multiple segments, and a segment is the result of multiple transactions acting together. A segment can therefore be represented as `[T_start, T_end]`, where `T_start` is the commit time of the earliest transaction and `T_end` is the commit time of the latest transaction. Since a segment can be compressed or merged into a new segment, another dimension is required to distinguish versions: `([T_start, T_end], [T_create, T_drop])`, where `T_create` is the creation time of the segment and `T_drop` is its deletion time. `T_drop = 0` indicates that the segment has not been discarded. A block uses the same representation. When a transaction commits, its read view needs to be obtained based on the commit time:
