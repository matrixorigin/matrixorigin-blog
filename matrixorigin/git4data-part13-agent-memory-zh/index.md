...
**做法一：提示词与 Markdown 文件。** 最普遍，`AGENTS.md`、`CLAUDE.md` 和各类 rules 文件都是。它透明、易编辑、随代码进 Git，非常适合编码规范、常用命令、架构原则这类**变化较慢的护栏**——Claude Code 的官方文档也是这么定位它的。代价是**它扛不住"一直在变"的那部分**：内容要人工维护，而在一个每天演进的项目里，你刚重构完目录、换了状态管理库，有几次真的会切出去把它记下来？于是旧规则悄悄失效，没有编译器报错，文件只是安静地对 Agent 撒谎。文件变长之后也只能全量加载或人工拆分，不同生命周期和权限的内容更是没法独立管理。

**做法二：对话历史 + 自动摘要。** 最近的消息留在窗口里，较早的压成摘要。实现简单，适合维持一次任务内的连续性。代价是**摘要是有损压缩**：会话一长，早期细节、因果关系和少数关键约束会被逐步磨掉；而且它不支持精确查询、选择性更新和跨用户隔离——你没法说"只把这一条改掉"。

**做法三：向量库 + RAG。** 把历史对话、笔记和经验切块嵌入，按语义相关性召回，避免每次全量加载。这是处理大规模非结构化记忆的有效办法，也确实解决了按需检索。代价是**相似度只能回答"哪段看起来相关"**：它不天然回答"哪条是最新事实""两条记忆是否冲突""是谁写的""能否撤销某次写入"。如果只追加不更新，"我们用 PostgreSQL"和"测试用 SQLite"会同时躺在库里，检索到哪条全看运气。

**做法四：结构化数据库或知识图谱。** 把用户、项目、实体、关系、时间和来源显式建模，于是精确过滤、冲突检查、权限控制、统计分析都成立了，知识图谱还特别适合表达复杂关系。代价是**要设计 schema、实体解析和更新策略**；而且光有结构化存储，语义检索和版本治理这两件事还是没解决。

**做法五：专用 Memory 框架或平台内建记忆。** Letta 这类框架会把常驻记忆块、文件和可检索的归档记忆组合起来，让 Agent 通过工具主动管理；GitHub 的 Copilot Memory 更进一步——**记忆在使用前先验证**，检查它引用的代码是否还存在，长期未验证的自动过期。这类方案接入成本最低，也说明整个行业正在从"存更多文本"走向"验证、更新和治理记忆"。代价是**绑定**：记忆跟着工具走，换个 Agent 不跟着；治理策略是平台定的，你能调的空间有限。
...
**能追溯**——知道哪个用户、哪个 Agent、哪次任务在什么时候写了什么。**能裁决**——识别重复、过期和互相矛盾的记忆，并且**保留必要的历史**，而不是一删了之。**能隔离**——高风险的写入先进隔离区，确认了变化范围再生效。
...
[Memoria](https://github.com/matrixorigin/Memoria) 是我们基于 MatrixOne 构建的开源 Agent Memory 项目（Apache-2.0）。做它的出发点很简单：
