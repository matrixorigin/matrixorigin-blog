...
我们在一台**单机 Docker（MatrixOne 4.0.0）**上，把同一张表分别灌到 100 万、1000 万、1 亿行，每一档都跑同一组 git4data 操作（其中 diff / merge 都**只改其中 1000 行**）。实测（稳定态，多次取中位数）：
...
| **1 亿行** | 41 s | 5 ms | 25 ms | 19 ms | 23 ms | 1.3 s |
...
- **diff / merge：只随“改了多少行”走**——三档都只改了 1000 行，`DIFF` 始终一二十毫秒；`MERGE` 会随表变大而增长（它要把变更写回主表），但这部分增长正比于写入工作量，是可以预期的。
