...
到这里，"快照为什么便宜"几乎是显而易见的了。

> **给一张表打快照，本质上就是复制它当前那份元数据目录。**

- `CREATE SNAPSHOT v1 FOR TABLE …`——把当前的目录结构保存下来、命名为 `v1`。目录只是一组"指向哪些对象"的引用，**与对象里装了 100 万行还是 1 亿行无关**，所以快照永远是几毫秒，与数据量无关。（一个细节：命名快照前，系统会先把还驻留在内存、未落盘的数据 flush 成对象——这就是上一篇提到的"第一次快照略慢"的来源，属于一次性开销。）
- 时间戳快照 `T{mo_ts='2025-09-12 12:34:56'}` 则更直接：无需提前保存，读取时按 MVCC 时间戳过滤目录中的对象即可，对应 Git 的一次 commit。
- **克隆**（`CLONE` / `DATA BRANCH CREATE`）同理——**只复制目录结构**，不复制任何数据对象。克隆出的新表与原表，从此各自在自己的目录上增删对象、互不影响，但起始时它们**共享同一批底层数据对象**。这正是"克隆 6 亿行只要 0.2 秒、仅多占 314 KB"的全部原因：那 314 KB 是新复制的目录，6 亿行数据一个字节都没有移动。

![快照与克隆：只复制目录，共享底层对象](./images/fig_snapshot-clone_zh.svg)
*快照和克隆都只复制目录，几份目录指向同一批不可变对象——所以 6 亿行也只要 0.2 秒。*

这里还有一个容易被忽略、却很关键的设计——**垃圾回收（GC）感知快照**。MatrixOne 平时会在后台做 compaction、回收不再需要的旧对象；但**被命名快照引用的对象，绝不会被回收**。正因如此，你派生一个分支、保留一个快照，才是真正"零成本"的：系统会替你把它依赖的对象钉住，不会在背后悄悄清理掉。

至此，前两篇说的"零成本分支""秒级版本"就都落到了实处——**因为它们根本没有移动数据，只复制了一份目录。**
...
> **所以要 diff `sn2` 和 `sn3`，MatrixOne 只需读取各自的增量 Δ_sn2 和 Δ_sn3，完全不必扫描两张全表。**
...
万一共同祖先已不可得呢（例如原表及其快照都被删除）？MatrixOne 会退化为以**空的共同祖先**来计算，结果依然正确；而且只要两张表确实同源，它们仍**共享大量底层对象**，聚合时**直接跳过两边共有的对象**即可，依然很快。

这一点恰好点出了 git4data 与"自己写 SQL 比对两张表"的本质差距：后者每次都要**扫描两张全表**；而 git4data 因为记录了血缘、又只保存增量对象，**始终只读取那一小部分变更**。
...
> 打快照，只是复制一份**元数据目录**，所以与数据量无关；克隆，只是复制目录、共享同一批对象，所以 6 亿行也只要 0.2 秒；diff/merge，只读取**增量对象 Δ**、用带 ± 号的聚合判定真假冲突，所以只随改动行数变化。**不可变对象 + 一份目录 + 只读增量**——版本控制就是从这三件事里自然生长出来的。

这也解释了一件更大的事：为什么最终是**数据库**、而非文件版本工具，扛起了"海量数据的版本控制"。因为只有数据库这种"既理解每一行的语义、又能把改动表达成不可变增量"的系统，才能让分支、diff、合并在 TB 级数据上廉价到可以随手就用。

下一篇，我们把视角从"如何实现"转回"用它做什么"——回到这个系列开篇就埋下的那条线：**让会自我进化的 AI agent，在版本化的数据上安全地探索、评估、合并、回滚。** 那才是 git4data 真正想去的地方。
